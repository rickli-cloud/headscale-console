...
// Source: https://github.com/Devolutions/IronRDP/blob/bdde2c76ded7315f7bc91d81a0909a1cb827d870/crates/ironrdp-web/src/clipboard/mod.rs
...
impl From<ClientFormatDescriptor> for ClipboardFormat {
    fn from(descriptor: ClientFormatDescriptor) -> Self {
        ClipboardFormat::new(descriptor.id)
            .with_name(ClipboardFormatName::new_static(descriptor.name))
    }
}
...
const FORMAT_WIN_HTML: ClientFormatDescriptor =
    ClientFormatDescriptor::new(FORMAT_WIN_HTML_ID, FORMAT_WIN_HTML_NAME);
const FORMAT_MIME_HTML: ClientFormatDescriptor =
    ClientFormatDescriptor::new(FORMAT_MIME_HTML_ID, FORMAT_MIME_HTML_NAME);
const FORMAT_PNG: ClientFormatDescriptor =
    ClientFormatDescriptor::new(FORMAT_PNG_ID, FORMAT_PNG_NAME);
const FORMAT_MIME_PNG: ClientFormatDescriptor =
    ClientFormatDescriptor::new(FORMAT_MIME_PNG_ID, FORMAT_MIME_PNG_NAME);
...
impl WasmClipboardMessageProxy {
    ...
    pub(crate) fn send_cliprdr_message(&self, message: ClipboardMessage) {
        if self
            .tx
            .unbounded_send(RdpInputEvent::Cliprdr(message))
            .is_err()
        {
...
impl WasmClipboard {
    pub(crate) fn new(
        message_proxy: WasmClipboardMessageProxy,
        js_callbacks: JsClipboardCallbacks,
    ) -> Self {
        Self {
            ...
            if format.id().is_registered() {
                if let Some(name) = format.name() {
                    ...
                    if !SUPPORTED_FORMATS
                        .iter()
                        .any(|supported| *supported == name.value())
                    {
                        // Unknown format
                        continue;
                    }
                    ...
                    let skip_mime_png = format_name_eq(format, FORMAT_MIME_PNG.name)
                        && formats
                            .iter()
                            .any(|format| format_name_eq(format, FORMAT_PNG.name));
                    ...
                    self.remote_mapping
                        .insert(format.id(), name.value().to_owned());
                }
            } else {
    ...
    pub(crate) fn process_event(
        &mut self,
        event: WasmClipboardBackendMessage,
    ) -> anyhow::Result<()> {
        match event {
...
impl CliprdrBackend for WasmClipboardBackend {
    ...
    // `fn on_ready` is not a member of trait `CliprdrBackend`
    ...
    fn on_format_data_request(&mut self, request: FormatDataRequest) {
        self.send_event(WasmClipboardBackendMessage::RemoteDataRequest(
            request.format,
        ));
    }

    fn on_format_data_response(&mut self, response: FormatDataResponse<'_>) {
        self.send_event(WasmClipboardBackendMessage::RemoteDataResponse(
            response.into_owned(),
        ));
    }
